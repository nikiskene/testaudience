...
-- Synthetic Mind Engine v3: real-world attention funnel + 0-10 activation ladder.
...
begin
  ...
  with eligible as (
    ...
    select f.*,
      greatest(0,least(100,round(
        9 + email_attention*.17 + curiosity*.08 + subject_specificity + subject_length - subject_generic_penalty
        - skepticism*.08 - case when seniority in('executive','senior_leadership','partner') then 7 else 2 end + jitter*.55
      )))::int attention,
      greatest(0,least(100,round(
        16 + curiosity*.13 + intellectual_stimulation_need*.11 + story_response*.07 + story_bonus + evidence_bonus*.45 + information_gap_bonus
        - skepticism*.10 - brochure_penalty - hype_penalty*(1-bullshit_tolerance/100.0) + jitter*.55
      )))::int read_pull,
      greatest(0,least(100,round(
        17 + intellectual_stimulation_need*.13 + experiential_learning_preference*.10 + specificity_bonus + concrete_offer_bonus
        ...
        + case when audience_segment='consultant' and consultant_signal=1 then curiosity*.11 else 0 end
        - brochure_penalty*.65 + jitter*.4
      )))::int relevance,
      ...
      greatest(0,least(100,round(16+story_response*.16+emotional_messaging_response*.10+story_bonus*.5+
        case when audience_segment='corporate_refugee' and refugee_signal=1 then unused_potential_feeling*.18 else 0 end-brochure_penalty*.4+jitter*.3)))::int emotional
    from features f
  ), scored as (
    select q.*,
      (attention>=44) as opened_flag,
      (attention>=44 and read_pull>=45) as read_flag,
      greatest(0,least(100,round(attention*.15+read_pull*.18+relevance*.24+credibility*.18+novelty*.08+emotional*.06+
        willingness_to_travel*.04+discretionary_spending_power*.04-price_sensitivity*.06-approval_complexity*.04)))::int interest,
      greatest(0,least(100,round(12+willingness_to_travel*.13+discretionary_spending_power*.13+decision_authority*.07+concrete_offer_bonus
        -price_sensitivity*.15-approval_complexity*.11+jitter*.2)))::int action_readiness
    from funnel q
  ), activated as (
    select x.*,
      case
        ...
        when interest<64 or credibility<48 then 7
  ...
  select p_sim_id,id,
    ...
    case activation
      when 0 then 'The subject line did not earn an open'
      when 1 then 'Maybe interesting for someone else, but not worth my time'
      when 2 then 'Maybe interesting for someone I know'
      when 3 then 'Potentially relevant at a different stage in my life or career'
      when 4 then 'Potentially interesting sometime in the future'
      when 5 then 'I would need time to think about it'
      when 6 then 'I may want more information later, but not now'
      when 7 then 'Tell me more before I decide whether this deserves time'
      when 8 then 'This sounds genuinely interesting'
      when 9 then 'This is relevant enough to justify a conversation'
      else 'The value and timing are strong enough for me to commit' end,
    ...
    case activation
      when 0 then 'Not interesting.'
      when 1 then 'Maybe interesting for someone else.'
      when 2 then 'Maybe interesting for someone I know.'
      when 3 then 'Maybe interesting at a different stage in my life.'
      when 4 then 'Maybe interesting somewhere in the future.'
      when 5 then 'Let me think about it.'
      when 6 then 'Maybe I want more information in the future.'
      when 7 then 'Tell me more.'
      when 8 then 'Sounds interesting.'
      when 9 then 'Let''s talk.'
      else 'OK, I am in.' end,
...
create or replace function public.lms_simulation_summary(p_sim_id uuid)
returns table(
...
begin
  ...
  return query select
    count(*),
    count(*)filter(where coalesce(activation_score,0)<=4),
    count(*)filter(where activation_score between 5 and 6),
    count(*)filter(where activation_score between 7 and 9),
    count(*)filter(where activation_score=10),
    round(100.0*count(*)filter(where activation_score>=7)/nullif(count(*),0),1),
    round(100.0*count(*)filter(where opened)/nullif(count(*),0),1),
    round(100.0*count(*)filter(where read_enough)/nullif(count(*),0),1),
    round(avg(activation_score),2),
    count(*)filter(where activation_score=0),count(*)filter(where activation_score=1),count(*)filter(where activation_score=2),
    count(*)filter(where activation_score=3),count(*)filter(where activation_score=4),count(*)filter(where activation_score=5),
    count(*)filter(where activation_score=6),count(*)filter(where activation_score=7),count(*)filter(where activation_score=8),
    count(*)filter(where activation_score=9),count(*)filter(where activation_score=10),
